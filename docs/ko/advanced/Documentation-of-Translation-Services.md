...
> [!CAUTION]
> 다음 내용은 pdf2zh 버전 1.x 를 위해 준비되었으며, 아직 버전 2.x 에 맞춰 업데이트되지 않았습니다. 따라서 정보가 **구식**일 수 있습니다.
>
...
| **OpenAI**           | `openai`       | `OPENAI_BASE_URL`, `OPENAI_API_KEY`, `OPENAI_MODEL`                   | `https://api.openai.com/v1`, `[Your Key]`, `gpt-4o-mini` | [OpenAI](https://platform.openai.com/docs/overview) 문서 참조                                                                                                                                                                    |
...
| **Ali Qwen 번역** | `qwen-mt`  | `ALI_MODEL`, `ALI_API_KEY`, `ALI_DOMAINS`                             | `qwen-mt-turbo`, `[Your Key]`, `scientific paper`        | 번체 중국어는 아직 지원되지 않으며, 간체 중국어로 번역됩니다. 자세한 내용은 [Qwen MT](https://bailian.console.aliyun.com/?spm=5176.28197581.0.0.72e329a4HRxe99#/model-market/detail/qwen-mt-turbo) 참조 |

위의 표에 나열되지 않았지만 OpenAI API 와 호환되는 대규모 언어 모델의 경우, 표에서 OpenAI 에 대해 설명된 동일한 방법으로 환경 변수를 설정할 수 있습니다.
