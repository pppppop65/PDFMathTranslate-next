...
在命令行中執行翻譯指令，於當前工作目錄生成翻譯後的文檔 `example-mono.pdf` 與雙語文檔 `example-dual.pdf`。預設使用 Google 作為翻譯服務。更多支援的翻譯服務可參閱[此處](https://github.com/PDFMathTranslate/PDFMathTranslate-next/blob/main/docs/ADVANCED.md#services)。
...
| `--custom-system-prompt`        | 用於翻譯的自定義系統提示。在 Qwen 3 中用於 `/no_think`                   | `pdf2zh example.pdf --custom-system-prompt "/no_think You are a professional, authentic machine translation engine"` |
...
| `--no-mono`                     | 不輸出單一語言 PDF 文件                                                    | `pdf2zh example.pdf --no-mono`                                                                                       |
...
| `--enhance-compatibility`       | 啟用所有兼容性增強選項                                           | `pdf2zh example.pdf --enhance-compatibility`                                                                         |
| `--use-alternating-pages-dual`  | 對雙頁 PDF 使用交替頁面模式                                                | `pdf2zh example.pdf --use-alternating-pages-dual`                                                                    |
| `--watermark-output-mode`       | PDF 檔案的水印輸出模式                                                    | `pdf2zh example.pdf --watermark-output-mode "NoWaterMark"`                                                           |
| `--max-pages-per-part`          | 分塊翻譯時每部分的最大頁數                                           | `pdf2zh example.pdf --max-pages-per-part 1`                                                                          |
...
| `--auto-enable-ocr-workaround`  | 啟用自動 OCR 解決方案。若偵測到文件為重度掃描檔，將嘗試啟用 OCR 處理並跳過後續掃描偵測。詳情請參閱文檔。(預設：False) | `pdf2zh example.pdf --auto-enable-ocr-workaround True`                    |
...
| `--welcome-page`                | 歡迎頁面 html 檔案的路徑          | `pdf2zh --gui --welcome-page /path`             |
...
你可以將命令行參數中的 `--` 替換為 `PDF2ZH_`，使用 `=` 連接參數，並將 `-` 替換為 `_` 作為環境變數。
...
強烈建議參考此設定檔的內容，並使用**使用者自訂設定檔**來實作自己的設定檔。

> [!TIP]
> - 預設情況下，pdf2zh 2.0 每次在圖形介面中點擊翻譯按鈕時，會自動將當前配置保存到 `~/.config/pdf2zh/config.v3.toml`。此配置文件將在下次啟動時自動載入。
...
當此參數設為 True 時，將跳過 PDF 清理步驟，這能提升相容性並避免部分字體處理問題。
...
PDFMathTranslate 會快取已翻譯的文本以提高速度，並避免對相同內容進行不必要的 API 調用。你可以使用 `--ignore-cache` 選項來忽略翻譯快取並強制重新翻譯。
...
當將 pdf2zh 圖形介面部署在公共服務上時，您應按照以下說明修改配置檔案。
...
當使用認證與歡迎頁面來指定哪位使用者可以使用 Web UI 並自訂登入頁面時：

範例 auth.txt
每行包含兩個元素，使用者名稱與密碼，以逗號分隔。
