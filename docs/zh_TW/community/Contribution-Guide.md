...
> [!CAUTION]
>
> 當前專案維護者正在研究自動化文檔國際化方案，因此任何與文檔國際化/翻譯相關的 PR 將不被接受！
>
> 請勿提交與文檔國際化/翻譯相關的 PR！
...
> [!NOTE]
>
> 如果你想貢獻文檔，請**僅修改英文版本文檔**。其他語言版本由貢獻者自行翻譯。
...
6. 在 GitHub 上建立 PR，提供詳細說明，並請求 [@awwaawwa](https://github.com/awwaawwa) 進行審查。
7. 確保所有自動化檢查通過。
...
- 請從 `main` 分支進行 fork，並在您 fork 的分支上進行開發。
   ...
   - 如果您的 PR 未通過自動檢查（顯示為 `checks failed` 和紅色叉號），請查看對應的 `details` 並修改您的提交，以確保新的 PR 通過所有檢查。
...
- 如果你引入了新的依賴項，請及時更新 `pyproject.toml` 文件中的依賴列表。


<h4 id="docupdate">5. 文件更新</h4>

- 如果您新增了命令行選項，請相應地在所有語言版本的 `README.md` 文件中更新命令行選項列表。
