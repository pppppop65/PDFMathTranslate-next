...
> [!CAUTION]
> 以下内容是为 pdf2zh 1.x 版本准备的，尚未针对 2.x 版本进行更新。因此，信息可能已**过时**。
>
...
我们已针对每种翻译服务所需的[环境变量](https://chatgpt.com/share/6734a83d-9d48-800e-8a46-f57ca6e8bcb4)提供了详细表格。请确保在使用相应服务前进行设置。
...
| **OpenAI-Liked**     | `openailiked`  | `OPENAILIKED_BASE_URL`, `OPENAILIKED_API_KEY`, `OPENAILIKED_MODEL`    | `url`, `[Your Key]`, `model name`                        | 无                                                                                                                                                                                                                       |
| **阿里通义翻译** | `qwen-mt`  | `ALI_MODEL`, `ALI_API_KEY`, `ALI_DOMAINS`                             | `qwen-mt-turbo`, `[Your Key]`, `scientific paper`        | 目前不支持繁体中文，将自动翻译为简体中文。详见[通义 MT](https://bailian.console.aliyun.com/?spm=5176.28197581.0.0.72e329a4HRxe99#/model-market/detail/qwen-mt-turbo) |

对于与 OpenAI API 兼容但未在上表中列出的大语言模型，您可以使用与表格中 OpenAI 相同的方法设置环境变量。
