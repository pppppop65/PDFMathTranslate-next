...
| `--max-pages-per-part`          | 拆分翻译时每部分的最大页数                                           | `pdf2zh example.pdf --max-pages-per-part 1`                                                                          |
...
| `--welcome-page`                | 欢迎页面 html 文件的路径          | `pdf2zh --gui --welcome-page /path`             |
...
> 
> 同理，`-25` 包含第 25 页之前的所有页面，相当于 `1-25`。

- 如果页面不是连续的，可以用逗号 `,` 分隔它们。
...
- 如果页面同时包含连续和非连续范围，也可以用逗号连接，如下所示：
