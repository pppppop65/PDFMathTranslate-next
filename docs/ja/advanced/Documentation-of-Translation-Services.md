...
各翻訳サービスに必要な[環境変数](https://chatgpt.com/share/6734a83d-9d48-800e-8a46-f57ca6e8bcb4)について、詳細なテーブルを提供しています。それぞれのサービスを使用する前に、必ず設定してください。

| **翻訳サービス**       | **サービス**    | **環境変数**                                             | **デフォルト値**                                       | **備考**                                                                                                                                                                                                                  |
| -------------------- | -------------- | --------------------------------------------------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| **OpenAI 互換**     | `openailiked`  | `OPENAILIKED_BASE_URL`, `OPENAILIKED_API_KEY`, `OPENAILIKED_MODEL`    | `url`, `[Your Key]`, `model name`                        | なし                                                                                                                                                                                                                       |
| **Ali Qwen Translation** | `qwen-mt`  | `ALI_MODEL`, `ALI_API_KEY`, `ALI_DOMAINS`                             | `qwen-mt-turbo`, `[Your Key]`, `scientific paper`        | 繁体中文はまだサポートされていません。簡体中文に翻訳されます。詳細は[Qwen MT](https://bailian.console.aliyun.com/?spm=5176.28197581.0.0.72e329a4HRxe99#/model-market/detail/qwen-mt-turbo)を参照してください |

上記の表に記載されていないが OpenAI API と互換性のある大規模言語モデルの場合、表内の OpenAI と同様の方法で環境変数を設定できます。
