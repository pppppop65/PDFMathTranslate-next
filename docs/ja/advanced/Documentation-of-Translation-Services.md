...
利用可能な翻訳サービスとその使い方を確認するには、コマンドラインでヘルプメッセージを表示します。
...
> [!CAUTION]
> 以下の内容は pdf2zh のバージョン 1.x 向けに準備されたものであり、バージョン 2.x 向けにはまだ更新されていません。そのため、情報が**古くなっている**可能性があります。
>
...
| **Tencent**          | `tencent`      | `TENCENTCLOUD_SECRET_ID`, `TENCENTCLOUD_SECRET_KEY`                   | `[Your ID]`, `[Your Key]`                                | 詳細は[Tencent](https://www.tencentcloud.com/products/tmt?from_qcintl=122110104)を参照                                                                                                                                             |
...
上記の表に記載されていないが OpenAI API と互換性のある大規模言語モデルの場合、表内の OpenAI に対して説明されている方法と同じ方法で環境変数を設定できます。
