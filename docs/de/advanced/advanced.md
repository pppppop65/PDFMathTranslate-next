...
| `--<Services>`                  | [**Bestimmten Dienst**](./Dokumentation-der-Übersetzungsdienste.md) für die Übersetzung verwenden | `pdf2zh example.pdf --openai`<br>`pdf2zh example.pdf --deepseek`                                                     |
...
| `--qps`                         | QPS-Begrenzung für den Übersetzungsdienst                                              | `pdf2zh example.pdf --qps 200`                                                                                       |
...
| `--skip-scanned-detection`      | Überspringen der Scan-Erkennung                                                                 | `pdf2zh example.pdf --skip-scanned-detection`                                                                        |
...
| `--disable-gui-sensitive-input` | Deaktiviert die GUI-Eingabeempfindlichkeit | `pdf2zh --gui --disable-gui-sensitive-input`    |
...
- Wenn die Seitenzahlen fortlaufend sind, können Sie es wie folgt schreiben:
...
- Wenn die Seiten nicht aufeinanderfolgend sind, können Sie ein Komma `,` zur Trennung verwenden.
...
Beispielsweise können Sie den folgenden Befehl verwenden, um ein GUI-Fenster zu aktivieren:
...
Bitte modifizieren Sie die Konfigurationsdateien im `default`-Verzeichnis nicht. 
...
Bei Verwendung von Authentifizierung und Willkommensseite, um festzulegen, welcher Benutzer die Web-Oberfläche nutzen und die Login-Seite anpassen kann:

Beispiel auth.txt
Jede Zeile enthält zwei Elemente, Benutzername und Passwort, durch ein Komma getrennt.
