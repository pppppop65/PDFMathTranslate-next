...
| `--restore-offline-assets`      | Восстановить пакет оффлайн-ресурсов из указанного каталога                            | `pdf2zh example.pdf --restore-offline-assets /path`                                                                  |
...
>
> При изменении одного и того же параметра разными способами, программа будет применять изменения в соответствии с указанным ниже порядком приоритетов.
>
> Изменения с более высоким приоритетом перезаписывают изменения с более низким приоритетом.
>
...
> - Файлы конфигурации в директории `default` автоматически генерируются программой. Вы можете скопировать их для изменения, но пожалуйста, не изменяйте их напрямую.
...
При развертывании графического интерфейса pdf2zh в публичных сервисах следует изменить файл конфигурации, как описано ниже.
...
При использовании Аутентификация и приветственная страница для указания пользователя, который будет использовать веб-интерфейс, и настройки страницы входа:
...
Файл содержит три столбца. Вот демонстрационный файл глоссария:
...
Теперь вы можете загрузить собственный файл глоссария. После загрузки файла вы можете просмотреть его содержимое, нажав на его имя, и содержимое отобразится ниже.
