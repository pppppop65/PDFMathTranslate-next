...
| `--restore-offline-assets`      | Restaurer le package d'assets hors ligne depuis le répertoire spécifié                | `pdf2zh example.pdf --restore-offline-assets /path`                                                                  |
...
| `--rpc-doclayout`               | Adresse hôte du service RPC pour l'analyse de la mise en page du document              |                                                                                                                      |
...
| `--pool-max-worker`             | Nombre maximum de workers pour le pool de traduction. Si non défini, utilisera le qps comme nombre de workers | `pdf2zh example.pdf --pool-max-worker 100`                                                                |
...
| `--split-short-line`            | Forcer la séparation des lignes courtes en différents paragraphes                      | `pdf2zh example.pdf --split-short-line`                                                                              |
...
| `--dual-translate-first`        | En mode PDF double, prioriser le placement de la page traduite                                          | `pdf2zh example.pdf --dual-translate-first`                                                                                            |
...
| `--auto-enable-ocr-workaround`  | Active la solution de contournement OCR automatique. Si un document est détecté comme étant fortement scanné, cette option tentera d'activer le traitement OCR et ignorera toute détection supplémentaire de scan. Voir la documentation pour plus de détails. (par défaut : False) | `pdf2zh example.pdf --auto-enable-ocr-workaround True`                    |
...
| `--welcome-page`                | Chemin vers le fichier html d'accueil  | `pdf2zh --gui --welcome-page /path`             |
...
Préserver les polices `Latex`, `Mono`, `Code`, `Italic`, `Symbol` et `Math` par défaut :
...
Invite personnalisé du système pour la traduction. Il est principalement utilisé pour ajouter l'instruction '/no_think' de Qwen 3 dans l'invite.
...
Dans la plupart des cas, vous pouvez directement passer vos paramètres souhaités via les arguments de ligne de commande. Veuillez vous référer à [Arguments de ligne de commande](#cmd) pour plus d'informations.
...
> - Les fichiers de configuration peuvent inclure des numéros de version tels que "v2", "v3", etc. Ce sont **des numéros de version de fichier de configuration**, **et non** le numéro de version de pdf2zh lui-même.
...
Le fichier contient trois colonnes. Voici un exemple de fichier de glossaire :
...
Vous pouvez utiliser plusieurs fichiers pour le glossaire. Les différents fichiers doivent être séparés par `,`.
